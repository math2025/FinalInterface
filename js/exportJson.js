function setupExportListeners() {
  ...
  // Remove existing event listeners to prevent multiple downloads
  ...
  newExportJsonButton.addEventListener("click", () => {
    ...
    document.querySelectorAll(".question-box").forEach((box, index) => {
      // Get question content from Tiptap instance
      ...
      // Retrieve options from Tiptap instances
      const options = [];
      box.querySelectorAll(".ck-option").forEach((optionDiv) => {
        ...
        if (optionEditorInstance) {
          ...
          options.push(""); // fallback to empty string
        }
      });
