function generateFileName(fileType) {
  ...
  // document.addEventListener("DOMContentLoaded", async function () {
  //   if (window.scriptLoaded) return; // 🔥 Prevent duplicate execution
  ...
  //   // 🟢 Function to remove existing event listeners before adding new ones
  ...
  //   // 📌 Export JSON (Fixes Double Download)
  //   const jsonButton = removeExistingEventListeners("export-json");
  ...
  //   // 📌 Export LaTeX (Fixes Double Download)
  //   const latexButton = removeExistingEventListeners("export-latex");
  ...
  //   // 📌 Export PDF (Fixes Double Download)
  //   const pdfButton = removeExistingEventListeners("export-pdf");
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    function createQuestionBlock(questionData = null) {
      ...
      questionBox
        .querySelector(".remove-question")
        .addEventListener("click", function () {
          questionHistory.push(questionBox); // Store actual element for undo
          questionBox.remove();
          showStatusMessage(
            "❌ Question deleted! Click 'Undo' to restore.",
            "error"
    ...
    undoActionButton.addEventListener("click", function () {
      if (questionHistory.length > 0) {
        const lastDeletedQuestion = questionHistory.pop();
        questionContainer.appendChild(lastDeletedQuestion);
        showStatusMessage("🔄 Last action undone!");
      } else {
        showStatusMessage("⚠️ No action to undo!", "error");
      }
    ...
    removeExistingEventListeners("export-json").addEventListener("click", function () {
      console.log("Exporting JSON...");
  ...
  removeExistingEventListeners("export-latex").addEventListener("click", function () {
    console.log("Exporting LaTeX...");
    ...
    document.querySelectorAll(".question-box").forEach((questionBox, index) => {
        let questionText = questionBox.querySelector(".question-input").value.trim();
        let difficulty = questionBox.querySelector(".difficulty").value;
        let options = [];
  ...
  removeExistingEventListeners("export-pdf").addEventListener("click", async function () {
    console.log("Exporting PDF...");
    try {
        const { jsPDF } = window.jspdf;
        const pdf = new jsPDF();
        ...
        document.querySelectorAll(".question-box").forEach((questionBox, index) => {
            ...
            pdf.setFontSize(12);
            pdf.text(`${index + 1}. ${questionText} (${difficulty.toUpperCase()})`, 10, y);
            y += 10;
        ...
        pdf.save(`${document.getElementById("doc-title").value.trim()}_${document.getElementById("doc-author").value.trim()}_${document.getElementById("doc-date").value}.pdf`);
    } catch (error) {
        console.error("Error generating PDF:", error);
        alert("❌ Failed to generate PDF.");
    }
